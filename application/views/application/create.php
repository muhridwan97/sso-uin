                    ...
                    <div class="col-sm-5">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="version" name="version"
                                   placeholder="Put app initial version eg. v2.1.0" required maxlength="20"
                                   value="<?= set_value('version', 'v0.1') ?>">
                            <?= form_error('version') ?>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="order" id="order" required>
                                ...
                                <?php foreach ($applications as $application): ?>
                                    <option value="<?= $order ?>"<?= set_select('order', $order) ?>>
                                        <?= $order ?> - <?= $application['title'] ?>
                                    </option>
                                    <?php $order++ ?>
                                <?php endforeach; ?>
                                <?php if(empty($applications)): ?>
                                    <option value="1" selected>1 - First Application</option>
                                <?php else: ?>
                                    <option value="<?= $order ?>" selected><?= $order ?> - Last order</option>
                                <?php endif; ?>
                            </select>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="icon" name="icon"
                                   ...
                                   value="<?= set_value('icon', 'mdi-application') ?>">
                            <p class="form-text">
